...
//countdown
let endDate = new Date("1/23/2025 00:00:00").getTime();
...
loginbtn.addEventListener('click', () => {
 ...
 function togglePassword(x,y){
    var passwordX = document.getElementById(x);
    var eyeY = document.getElementById(y);
    if(passwordX.type === 'password'){
        passwordX.type = 'text';
        eyeY.classList.remove('fa-eye');
        eyeY.classList.add('fa-eye-slash');
    }
    else{
        passwordX.type ='password';
        eyeY.classList.add('fa-eye');
        eyeY.classList.remove('fa-eye-slash');
    }
 }
// Chức năng đăng ký
    ...
    if (fullNameUser && /^\d+$/.test(phoneUser) && phoneUser.length == 10 && passwordUser.length >= 8 && passwordConfirmation && checkSignup)  {
        if (passwordConfirmation == passwordUser) {
            ...
            if (!checkloop) {
                ...
                toast({ title: 'Thất bại', message: 'Só điện thoại này đã được sử dụng !',type: 'warning', duration: 2000 });
            }
        }
    }
}
...
loginButton.addEventListener('click', () => {
    ...
    if (phonelog && passlog) {
        ...
        if (vitri == -1) {
            toast({ title: 'Thất bại', message: 'Số điện thoại chưa được đăng ký !',type: 'warning', duration: 2000 });
...
function kiemtradangnhap() {
    ...
    if (currentUser != null) {
        ...
        document.querySelector('.auth-container').innerHTML = `<span class="text-dndk">Tài khoản</span>
            <span class="text-tk">${user.fullname} </span>`
...
// Phân trang 
let perPage = 12;
let currentPage = 1; // Biến toàn cục
...
function setupPagination(productAll, perPage) {
    ...
    // Đảm bảo rằng luôn có 5 trang được hiển thị
    while ((endPage - startPage + 1) < maxVisiblePages) {
        if (startPage > 1) {
            startPage--;
        } else if (endPage < page_count) {
            endPage++;
        } else {
            break;
        }
    ...
    if (page_count > 1) {
        ...
        if (currentPage > 1) {
            ...
            prev.innerHTML = `<a href="javascript:;"> <button class="changeBtn"> << </button> </a>`;
        ...
        // Thêm trang đầu tiên
        if (startPage > 1) {
            let firstPage = paginationChange(1, productAll);
            document.querySelector('.page-nav-list').appendChild(firstPage);
            if (startPage > 2) {
                let dots = document.createElement('li');
                dots.innerHTML = `<a href="javascript:;" class="custom-link">...</a>`;
                document.querySelector('.page-nav-list').appendChild(dots);
            }
        }
        ...
        if (endPage < page_count) {
            ...
            let lastPage = paginationChange(page_count, productAll);
            document.querySelector('.page-nav-list').appendChild(lastPage);
        }

        // nút tiến
        if (currentPage < page_count) {
            let next = document.createElement('li');
            next.innerHTML = `<a href="javascript:;"> <button class="changeBtn"> >> </button> </a>`;
...
function paginationChange(page, productAll) {
    ...
    node.addEventListener('click', function () {
        ...
        setupPagination(productAll, perPage); // Gọi lại hàm setupPagination sau khi thay đổi trang
        document.getElementById("home-service").scrollIntoView();
    })
    return node;
}
...
function xulyDathang(product) {
    ...
    document.querySelector('.form-message-checkout-ten').innerHTML = ' ! vui lòng chỉ nhập chữ cho tên';
    document.getElementById('tennguoinhan').focus();
    document.getElementById('tennguoinhan').value = '';
    }
    else {
        document.querySelector('.form-message-checkout-ten').innerHTML = '';
    }  
    if(sdtnhan.length == 0){
        document.querySelector('.form-message-checkout-sdt').innerHTML = ' ! vui lòng nhập số điện thoại';
        document.getElementById('sdtnhan').focus();
    } else if (!/^\d+$/.test(sdtnhan)) {
        document.querySelector('.form-message-checkout-sdt').innerHTML = ' ! vui lòng chỉ nhập số';
        document.getElementById('sdtnhan').focus();
        document.getElementById('sdtnhan').value = '';
    } else if (sdtnhan.length != 10) {
        document.querySelector('.form-message-checkout-sdt').innerHTML = ' ! vui lòng nhập số điện thoại 10 số';
        document.getElementById('sdtnhan').focus();
        document.getElementById('sdtnhan').value = '';
    } 
    else {
        document.querySelector('.form-message-checkout-sdt').innerHTML = '';
    }
    
    if(diachinhan.length == 0){
        document.querySelector(".form-message-checkout-diachi").innerHTML = '! vui lòng nhập địa chỉ nhận hàng';
        document.getElementById("diachinhan").focus();
    }
    else {
        document.querySelector('.form-message-checkout-diachi').innerHTML = '';
    }

    if (!/^[a-zA-ZÀ-ỹ]+$/.test(tennguoinhan) || sdtnhan.length !== 10 || !/^\d+$/.test(sdtnhan) || !diachinhan) {
